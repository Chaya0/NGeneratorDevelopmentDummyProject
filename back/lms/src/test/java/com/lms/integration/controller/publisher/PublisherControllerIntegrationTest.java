package com.lms.integration.controller.publisher;
...
class PublisherControllerIntegrationTest {
	...
	void testSearchPost() throws Exception {
		...
		mockMvc.perform(post("/api/publisher/search").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(searchDTO)))
			.andExpect(jsonPath("$[0].id").value(entity1.getId()))
			.andExpect(jsonPath("$[0].name").value(entity1.getName()))
			.andExpect(jsonPath("$[0].address").value(entity1.getAddress()))
			.andExpect(jsonPath("$[0].website").value(entity1.getWebsite()))
	...
	void testSearchPageablePost() throws Exception {
		...
		mockMvc.perform(post("/api/publisher/searchPageable").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(searchDTO)))
			...
			.andExpect(jsonPath("$.content[0].website").value(entity1.getWebsite()))
			.andExpect(jsonPath("$.size()").value(greaterThan(0)))
			.andExpect(status().isOk());
	}
	@Test
	void testDelete() throws Exception {
		mockMvc.perform(delete("/api/publisher/" + entity1.getId()))
			.andExpect(status().isNoContent());
	}
	@Test
	void testGetById() throws Exception {
		mockMvc.perform(get("/api/publisher/" + entity1.getId()).contentType(MediaType.APPLICATION_JSON))
	...
	void testInsert() throws Exception {
		...
		mockMvc.perform(post("/api/publisher").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(entity3)))
			.andExpect(jsonPath("$.name").value(entity3.getName()))
			.andExpect(jsonPath("$.address").value(entity3.getAddress()))
			.andExpect(jsonPath("$.website").value(entity3.getWebsite()))
			.andExpect(status().isOk());
	}
	@Test
	void testUpdate() throws Exception {
		entity1.setName("name1");
		mockMvc.perform(put("/api/publisher").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(entity1)))
			.andExpect(jsonPath("$.name").value(entity1.getName()))
			.andExpect(status().isOk());
	}
}
