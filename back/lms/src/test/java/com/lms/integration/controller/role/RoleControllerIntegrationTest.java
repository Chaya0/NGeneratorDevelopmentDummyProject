package com.lms.integration.controller.role;
...
class RoleControllerIntegrationTest {
	...
	void testSearchPost() throws Exception {
		...
		mockMvc.perform(post("/api/role/search").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(searchDTO)))
			.andExpect(jsonPath("$.size()").value(greaterThan(0)))
			.andExpect(status().isOk());
	}
	@Test
	void testSearchPageablePost() throws Exception {
		...
		mockMvc.perform(post("/api/role/searchPageable").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(searchDTO)))
			.andExpect(jsonPath("$.size()").value(greaterThan(0)))
			.andExpect(status().isOk());
	}
	@Test
	void testDelete() throws Exception {
		mockMvc.perform(delete("/api/role/" + entity1.getId()))
			.andExpect(status().isNoContent());
	}
	@Test
	void testGetById() throws Exception {
		mockMvc.perform(get("/api/role/" + entity1.getId()).contentType(MediaType.APPLICATION_JSON))
	...
	void testInsert() throws Exception {
		...
		mockMvc.perform(post("/api/role").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(entity3)))
	...
	void testUpdate() throws Exception {
		entity1.setName("name1" + UUID.randomUUID().toString());
		mockMvc.perform(put("/api/role").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(entity1)))
			.andExpect(jsonPath("$.name").value(entity1.getName()))
			.andExpect(status().isOk());
	}
}
